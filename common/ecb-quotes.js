...
export const EcbQuotesServiceProvider = function (anEventAggregator, anInformationHolder, mockMode = false) {
    ...
    eventAggregator.subscribe("quotesReceivedEcb", (eventArgs) => {
        ...
        const cs = [];
        // First quotes
        Object.keys(data.dataSets[0].series).forEach(key => cs.push({
            currency: "",
            quote: data.dataSets[0].series[key].observations[0][0]
        }));
        // Then currencies
